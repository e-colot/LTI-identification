...
%% Run AIC on stabilized ML
    ...
    MLest = @(Na, Nb) stableRealML(G_BLA, total_var, f, Na, Nb);
...
%% Plots
    ...
    subplot(211);
        legend('ML', 'ML with stable real poles');
        xlabel('Frequency [Hz]');
...
%% Position of poles and zeros
    ...
    plot(real(rootsB), imag(rootsB), 'o', LineWidth=2);
    hold on
    plot(real(A_optML), imag(A_optML), 'x', LineWidth=2);
