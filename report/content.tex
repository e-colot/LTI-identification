...
\begin{figure}
    \centering
    \includegraphics[width=0.48\textwidth]{Pictures/AIC_BTLS.png}
    \caption{AIC cost depending on the order of the transfer function for the BTLS estimator. The optimal order is circled in red.}
    \label{fig:AIC_3D}
\end{figure}
...
\begin{itemize}
    \item The LLS and TLS estimators both have a high number of parameters and Fig.~\ref{fig:rlocus1} confirms the hypothesis of imperfect pole-zero cancellations.
    \item GTLS has a more intermediate number of parameters.
    \item BTLS and ML estimators use a really low number of poles and zeros, yet achieve a performance similar to the GTLS on Fig.~\ref{fig:estimated_FRF}.
\end{itemize}
...
\begin{figure}
    ...
    \caption{Poles and zeros of the LLS and TLS estimators}
    \label{fig:rlocus1}
\end{figure}
...
Finally, the poles and zeros of the other estimator are shown in Fig.~\ref{fig:rlocus2}. It is here clearly visible that the GTLS estimator has poles in the right half-plane at high frequency, meaning the resulting system is unstable. Every estimator has poles with a positive real part but those of the ML and BTLS estimators are around $20$~Hz, which is by far \textit{less unstable} than the ones above $800$~Hz of the GTLS.
