...
More rigorously, the final ML cost of all the maximum likelihood estimators, computed with~(\ref{eq:ML_cost}), are shown in Table~\ref{tab:ML_costs} and it can be seen that the SRML performs a bit worse than the SML. This is logical as the SRML algorithm finds a minimal cost in a reduced solution set, which does not contain either the unstable solution of ML or the unreal solution of SML.
