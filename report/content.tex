...
The averaged FRF of a single realization $\hat{G}^{[m]}$ does not contain noise (at least in expected value and for zero-mean noise). But between realizations, they have different nonlinear effects as the phases of the excitation signals have changed. The variance between those is therefore due to nonlinear distortion only and averaging it out gives a FRF estimate that is noiseless and purely linear, if $M$ and $P$ are large enough to be close to the expected value.
\begin{gather}
...
It is here mentioned but as there is no use of the determined model, there is no interest in defining the category of excitations for which the model can be considered valid.
...
\paragraph{Use of the nonparametric estimate} All the estimators are relying on approaching~(\ref{eq:G_BLA_def}), which arises from the definition of a transfer function. The key idea is to notice that because
\begin{equation}
...
\paragraph{Forcing real valued parameters} A real system is described by real coefficients for a polynomial model~(\ref{eq:TF_parametric_model}) so $\theta$ must be real. The regressor must therefore be changed, meaning $y$ and $H$ are replaced by a concatenation of their real and imaginary parts called $y_{\text{re}}$ and $H_{\text{re}}$ as follows
\begin{equation}
...
The resulting standard deviation on the unstable pole (the one in the right half-plane) is large as shown in Table~\ref{tab:pole_stddev}. The pole is however far from the origin, which means it is unlikely to move to the left half-plane.
...
\begin{gather}
    ...
    \end{bmatrix}^T.
\end{gather}
This model makes sure the complex poles are complex conjugates, while keeping the parameters real. However, constraining the poles to the left half-plane is still done by limiting $\Delta \theta$ with~(\ref{eq:delta_theta_limit}), which is no longer following the gradient. The amplitude and phase parametrization makes the interpolation harder: only $\phi_k$ should be limited, except if the amplitude changes sign. This could make the estimator differ from the optimal stable solution, which needs to be solved.

Before adding a last layer of complexity to cure this, let's rapidly discuss the results of the SML and SRML estimators. First visually, the SRML estimate shown on Fig.~\ref{fig:FRF_SRML} has a worse phase match than Fig.~\ref{fig:FRF_SML}, while both seem to follow well the amplitude. It can be precised that the ML model is used as reference instead of the measured points as it is known that the ML estimate is mathematically optimal. 


More rigorously, the final ML cost of all the maximum likelihood estimators, computed with~(\ref{eq:ML_cost}), are shown in Table~\ref{tab:ML_costs} and it can be seen that the SRML performs a bit worse than the SML. This is expected as the SRML algorithm finds a minimal cost in a reduced solution set, which does not contain either the unstable solution of ML or the unreal solution of SML.
...
This norm was however not negligible with the OML estimator so the LM was implemented and "retro-fitted" to each ML estimator. The most complicated part was the determination of a convergence condition and it was decided to be a minimal threshold on the relative change of the parameters:
\begin{equation}
    \frac{||\Delta\theta||}{||\theta||} \leq 10^{-12}
\end{equation}

There is no cost improve between this estimator and the SRML, as shown in Table~\ref{tab:ML_costs} and they are in fact finding the same exact solution. This is once again explained by thinking about the available solution set, which is exactly the same for both.

This does not imply those are the same and they could differ slightly when poles approach the imaginary axis. As discussed in Sec.~\ref{sec:SRML}, the interpolation in case of constraints violation is badly handled and could therefore be suboptimal.
...
The optimal order of the ML estimators shown on Fig.~\ref{fig:optimal_order_ML} shows that the SRML and OML estimators have a larger order than the ML estimate. It is not a surprise and it even proves that the \textit{classical way} of stabilizing a system by symmetrizing unstable poles along the imaginary axis is not the optimal way, in which case the model order would have been the same for all the estimators.
...
And finally, the resulting standard deviation of the poles are given in Table~\ref{tab:ML_pole_stddev}. It can be observed that there is a factor $2$ between the standard deviation of the SRML and the one of OML, and this only for the real part of their complex poles. 


It is simply due to the squaring of $\alpha_k$ in~(\ref{eq:TF_OML}), which makes the link between pole position and parameter behave differently.
...
\begin{table}
    ...
    \begin{tabular}{l c c}
    \hline
    \textbf{Pole location} & \textbf{Std. deviation-SRML} & \textbf{Std. deviation-OML} \\
    \hline
    $-4250.10$ 
        & $275.23$ 
        & $275.23$ \\
